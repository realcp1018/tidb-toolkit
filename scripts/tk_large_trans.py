...
class Config(object):
    ...
    def parse(self):
        ...
        self.db = config["massive_dml"]["db"]
        self.table = config["massive_dml"]["table"]
        self.sql = config["massive_dml"]["sql"]
        self.start_rowid = config["massive_dml"].get("start_rowid", None)
        self.end_rowid = config["massive_dml"].get("end_rowid", None)
        self.batch_size = config["massive_dml"].get("batch_size", 1000)
        self.max_workers = config["massive_dml"].get("max_workers", 50)
        self.execute = config["massive_dml"].get("execute", False)
